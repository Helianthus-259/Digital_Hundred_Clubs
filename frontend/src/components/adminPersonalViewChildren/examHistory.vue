...
const search = () => {
  data.value = clubsData.value.filter((item) => {
    return (item.collegeReviewStatus === status.value || status.value === 'all') &&
...
const columns = ref([
  ...
  {
    colKey: 'collegeReviewStatus',
    title: '审批状态',
    cell: (h, { row }) => {
      return (
          <t-tag shape="round" theme={statusNameListMap[row.collegeReviewStatus].theme} variant="light-outline">
            {statusNameListMap[row.collegeReviewStatus].icon}
            {statusNameListMap[row.collegeReviewStatus].label}
          </t-tag>
      );
    },
  },
